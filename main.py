...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "6260622606:AAF6CzWlP0mrHpcqZZsEMhi-rNyXQLQ5SKs")
    API_ID = int(os.environ.get("API_ID",  "24262622"))
    API_HASH = os.environ.get("API_HASH", "50831eb3329ed9c0557aa2bc6aa34376")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '1477222048').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1001956100269').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = os.environ.get("LOG_CH", "-1002141623088")
